...
parser.add_argument('--model', default='base', type=str, help='model to evaluate')
...
class SegNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.cbd_sum_da = [atten_decoder[0][-1][-1], atten_decoder[1][-1][-1], atten_decoder[2][-1][-1]]
...
SegNet_MTAN.load_state_dict(model)

xAI = True
if xAI == True:
    # Train and evaluate multi-task network
    multi_task_tester(nyuv2_test_loader, SegNet_MTAN, device, model_name)
else:
    # Evaluate uncertainty
    make_preds(nyuv2_test_loader, SegNet_MTAN, device)
