...
def model_fit(x_pred, x_output, task_type):
    ...
    if task_type == 'semantic':
        ...
        print("Data type of x_pred: ", x_pred.dtype, "Data type of x_output: ", x_output.dtype)
...
def calculate_memory_usage(tensor):
...
def multi_task_trainer(train_loader, test_loader, multi_task_model, device, optimizer, scheduler, opt, total_epoch=200):
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            mem_data = 0
            mem_data += calculate_memory_usage(train_data)
            mem_data += calculate_memory_usage(train_label)
            mem_data += calculate_memory_usage(train_depth)
            mem_data += calculate_memory_usage(train_normal)
            print(f"Memory usage of train_data: {mem_data / (1024 ** 2)} MB")
            ...
            print("Mem Cached: ", torch.cuda.memory_reserved())
    ...
    # Save model
    torch.save(multi_task_model.state_dict(), "models/multi-task.pt")
...
def multi_task_mgd_trainer(train_loader, test_loader, multi_task_model, device,
    ...
    print("Training Multi Task MGD model...")
    start_time = time.time()
